function [T,time,sor_gauss_iter]= transient_sor_gauss(n1,n2,T_old,T_initial,T,x,y,tol,term1,term2,term3,alpha,sor_gauss_iter)
tic                                                                        %start of simulation time
    ...
    while(error>tol)                                                       %convergence loop
            ...
            sor_gauss_iter=sor_gauss_iter+1;
            ...
            colorbar;
            clabel(c,d)
            title_text = sprintf('Iteration number=%d for Steady state condition using SOR-Gauss_seidel Implicit Iterative Solver',sor_gauss_iter);
            title(title_text);
            xlabel('X')
            ylabel('Y')
            pause(0.03)
