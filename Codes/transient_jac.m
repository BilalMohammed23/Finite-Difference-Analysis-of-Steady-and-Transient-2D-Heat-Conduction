function [T,time,jac_iter]=transient_jac(n1,n2,T_old,T_initial,T,dx,dy,x,y,tol,term1,term2,term3,jac_iter)
...
for p=1:1400                                                               %time loop
    ...
    while(error>tol)                                                       %convergence loop
            ...
            colorbar;
            clabel(c,d);
            title_text = sprintf('Iteration number=%d for Steady state condition using Jacobian Implicit Iterative Solver',jac_iter);
            title(title_text);
            xlabel('X')
            ylabel('Y')
            pause(0.03)
